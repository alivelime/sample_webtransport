...
func main() {
	...
	sender := NewHlsSender()
	if err := watch(sender); err != nil {
		log.Println(err)
		return
	}
	log.Println("watchdog start.")
	...
	go func() {
		...
		wtErr <- wt.Serve(addr, certs, sender)
	}()
...
func (wt *WebTransportServer) Serve(addr string, certs []tls.Certificate, sender Sender) error {
	wt.sender = sender
...
func (wt *WebTransportServer) receiveClientConnect(sess quic.Session) (EventHandler, error) {
  ...
  stop := make(chan interface{})
	...
	if u.Path == "/video/stream" {
    conn = &WarpVideo{session: sess, sender: wt.sender, stop: stop}
	}
	if u.Path == "/audio/stream" {
    conn = &WarpAudio{session: sess, sender: wt.sender, stop: stop}
	}
	...
	go func() {
		for {
			buf := make([]byte, 16)
			if _, err = io.ReadFull(reqStream, buf); err != nil {
				log.Println(err)
        close(stop)
        return
			} else {
